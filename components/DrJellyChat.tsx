...
import { Send, MessageSquare, Brain, ArrowLeft } from './Icons'; // Added ArrowLeft
import { GenerateContentResponse } from '@google/genai';
import { getDrJellyChat as initChat } from '../services/geminiService'; // Renamed import

interface DrJellyChatProps { // Renamed interface
  logs: DailyLog[];
  onBack: () => void; // Added onBack prop
}

export const DrJellyChat: React.FC<DrJellyChatProps> = ({ logs, onBack }) => { // Renamed component and destructured onBack
  ...
  useEffect(() => {
    ...
    if (savedChat) {
      ...
      const initialMsg: ChatMessage = {
        id: 'init',
        role: 'model',
        text: "Hi there! I'm Jelly. How are you drifting today?", // Removed 'Dr.'
  ...
  return (
    <div className="glass-panel rounded-2xl h-[600px] flex flex-col animate-fade-in relative overflow-hidden">
      ...
      <div className="p-4 border-b border-white/10 flex items-center gap-3 z-10 bg-white/5 backdrop-blur-md">
        ...
        <div>
          <h2 className="font-bold text-lg text-white">Jelly</h2> {/* Removed 'Dr.' */}
